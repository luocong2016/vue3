import {
  ...
  onActivated, onBeforeMount, onMounted, onUnmounted,
  type ExtractPropTypes,
} from 'vue';
import {
  makeNumberProp, makeStringProp,
  makeRequiredProp, makeRequeiredDefaultProp
} from '@/utils/props';
...
const virtualProps = {
  dataKey: makeRequiredProp([String, Function]),
  dataSources: makeRequeiredDefaultProp<any>(Array, () => []),
  dataComponent: makeRequiredProp([Object, Function]),
  keeps: makeNumberProp(30),
  extraProps: Object,
  estimateSize: makeNumberProp(50),
  direction: makeStringProp<'vertical' | 'horizontal'>('vertical'),
  start: makeNumberProp(0),
  offset: makeNumberProp(0),
  topThreshold: makeNumberProp(0),
  bottomThreshold: makeNumberProp(0),
  pageMode: Boolean,
  rootTag: makeStringProp<keyof HTMLElementTagNameMap>('div'),
  wrapTag: makeStringProp<keyof HTMLElementTagNameMap>('div'),
  wrapClass: makeStringProp<keyof HTMLElementTagNameMap>('div'),
  wrapStyle: Object,
  itemTag: makeStringProp<keyof HTMLElementTagNameMap>('div'),
  itemClass: String,
  itemClassAdd: Function,
  itemStyle: Object,
  headerTag: makeStringProp<keyof HTMLElementTagNameMap>('div'),
  headerClass: String,
  headerStyle: Object,
  footerTag: makeStringProp<keyof HTMLElementTagNameMap>('div'),
  footerClass: String,
  footerStyle: Object,
...
enum EVENT_TYPE { ITEM = 'itemResize', SLOT = 'slotResize' }
enum SLOT_TYPE { HEADER = 'thead', FOOTER = 'tfoot' }
...
export default defineComponent({
  ...
  setup(props, { emit, slots, expose }) {
    ...
    const getRenderSlots = () => {
      const slots = [];
      const { start, end } = range.value as { start: number, end: number };
      const {
        dataSources,
        dataKey,
        itemClass,
