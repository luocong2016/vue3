...
import { addUnit, truthProp, numericProp, makeStringProp, createNamespace } from '@/utils';
...
export default defineComponent({
  ...
  setup(props, { slots }) {
    const hasContent = () => {
      ...
      return !_.isNil(content) && content !== '' && (showZero || content !== 0);
    };
    ...
    const renderContent = () => {
      ...
      if (!dot && hasContent()) {
        ...
        if (!_.isNil(max) && _.isNumber(content!) && +content > max) {
          return `${max}+`;
        }
        return content;
      }
    ...
    const style = computed(() => {
      const style: CSSProperties = { background: props.color };
      if (props.offset) {
