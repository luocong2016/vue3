...
export function trigger(
  target: HTMLElement | null | Ref<HTMLElement | undefined>,
  type: string,
  options?: EventInit // { bubbles: false, cancelable: false, composed: false }
) {
  // https://developer.mozilla.org/zh-CN/docs/Web/API/Event/Event
  let el: any = unref(target);
  if (!el) {
    return false;
  }
  el = el.$el || el;
  const ev = new Event(type, options);
