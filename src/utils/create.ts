...
export function createTranslate(name: string) {
  ...
  return (path: string, ...args: unknown[]) => {
    ...
    const message = _.get(messages, prefix + path) || _.get(messages, path);

    return _.isFunction(message) ? message(...args) : message;
  };
}
...
export function createNamespace(name: string) {
  const prefixedName = `${prefixe}-${name}`;
